...
form.addEventListener('submit', (e) => {
    ...
    if (isValid) {
        ...
        let reference = '';
        if (newId < 10) {
            reference = 'ID' + padStart(newId.toString(), 5, '1');
        }
        else if (newId > 9 && newId < 100) {
            reference = 'ID' + padStart(newId.toString(), 4, '1');
        }
        else if (newId > 99 && newId < 1000) {
            reference = 'ID' + padStart(newId.toString(), 3, '1');
        }
        ...
        let imageUrl = '';
        if (type === "AIR") {
            imageUrl = 'https://img.freepik.com/photos-gratuite/vue-avion-3d_23-2151022233.jpg?t=st=1717687681~exp=1717691281~hmac=df32197b75f21b09e5898bc2575756cdda7aefd6f1f98547dc6e6ddc073e9cfb&w=900';
        }
        else if (type === "MARITIME") {
            imageUrl = 'https://img.freepik.com/photos-gratuite/icone-voyage-3d-bateau_23-2151037325.jpg?t=st=1717687515~exp=1717691115~hmac=552f4045c94f263c61025520c64fb8695603ab53d4f94e68ccf99418b42b08b6&w=360';
        }
        else {
            imageUrl = 'https://img.freepik.com/photos-premium/illustration-conception-camion-minimaliste_7023-270197.jpg?w=740';
        }
    ...
    fetch('api.php?action=getCargaison', {
        ...
        paginatedCargaisons.forEach(cargaison => {
            if (cargaison) {
                ...
                <td>${cargaison.globalState}</td>
                <td>${cargaison.progressionState}</td>
                <td class="flex justify-center items-center "><button type="button" class="add-product-btn  p-2 text-2xl " data-id="${cargaison.id}">+</button>
                <button type="button" class="add-product-btn flex justify-center items-center rounded-full w-6 h-6 bg-yellow-500 p-2 text-2xl " data-id="${cargaison.id}">!</button>
                </td>
                `;
                tbodyCargo.appendChild(row);
            }
